...
class Ftep_Functions_filters {
    ...
    public function prepareFilters($user, $model, $params) {
        ...
        $filterFtep = $this->prepareFtepFilter();
        $filters[] = $filterFtep;
        return $filters;

    }
    protected function prepareFtepFilter( ){
    ...
    private function prepareFilterQuery_contextualSearch($user, $model) {
        ...
        return ' LOWER(' . $model->properties['visibility']['name'] . ') IN (' . implode(',', 'LOWER(' . $visibilities . ')') . ')';
