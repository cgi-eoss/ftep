...
@Slf4j
public class QgisRdp extends AbstractWrapperProc {

    public QgisRdp(String dockerImgName) {
        super(dockerImgName);
    }

    private static final String DOCKER_IMAGE_NAME = "ftep-qgis";

    @SuppressWarnings({"rawtypes", "unchecked"})
    public static int QGIS(HashMap conf, HashMap inputs, HashMap outputs) {

        QgisRdp s2App = new QgisRdp(DOCKER_IMAGE_NAME);
        ...
        if (s2App.isSufficientCoinsAvailable()) {
            ...
            if (null == portBinding) {
                LOG.error("Cannot find a port to start the QGIS application");
                return ZooConstants.WPS_SERVICE_FAILED;
            }
            ...
            if (null != hostPort) {
                LOG.info("Found a free port. QGIS application will start on port: {} for job: {}",
                        hostPort, jobID);
            } else {
                LOG.error("Cannot find a port to start the QGIS application");
                return ZooConstants.WPS_SERVICE_FAILED;
            ...
            LOG.debug("Updating GUI endpoint for QGIS application job: {}", jobID);
