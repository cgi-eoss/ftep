    ...
    node('docker') {
        ...
        buildImg.inside(dockerArgs) {
            wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'XTerm']) {
                // Inject a new trusted certificate for the build environment
                configFileProvider([configFile(fileId: '0decb4b0-628a-433b-ad74-ea1506b4032c', variable: 'TRUSTED_CERT')]) {
                    sh """
                        update-ca-trust
                        cp ${TRUSTED_CERT} /etc/pki/ca-trust/source/anchors/ftep-build.crt
