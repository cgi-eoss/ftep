...
public interface JobsApi extends BaseRepositoryApi<Job>, JobsApiCustom, PagingAndSortingRepository<Job, Long> {
    ...
    @Query("select t from Job t where (t.id like %:filter% or t.config.label like %:filter%) and t.status in (:status)")
    ...
    @Query("select t from Job t where (t.id like %:filter% or t.config.label like %:filter%) and t.status in (:status) and t.owner=:owner")
    ...
    @Query("select t from Job t where (t.id like %:filter% or t.config.label like %:filter%) and t.status in (:status) and not t.owner=:owner")
