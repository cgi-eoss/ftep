...
public class CacheManager {
  ...
  public boolean checkIfUrlIsInRecentsList(String singleUrl) {
    System.out.println(
        "fn name: CacheManager.checkIfUrlIsInRecentsList(); params: singleUrl '" + singleUrl + "'");
  ...
  public String getSymlinkForAlreadyExistingUrl(String oneUrlItem, String jobdir) {
    System.out
        .println("fn name: CacheManager.getSymlinkForAlreadyExistingUrl(); params: oneUrlItem '"
            + oneUrlItem + "' jobdir '" + jobdir + "'");
  ...
  public String createSymlink(String fileNameInCache, String targetJobdirForSymlinks) {
    ...
    try {
      ...
      String targetFile = Variables.TARGET_ROOT_PATH + targetJobdirForSymlinks + slashFileName;
      ...
      // System.out.println("Target of link " + linkTarget + " is " +
      // java.nio.file.Files.readSymbolicLink(linkTarget));
    } catch (Exception e) {
  ...
  public String unzipFile(String zipFile) {
    ...
    // check if ZIP file, if not don't unzip it!
    ...
    if (zipFile.toLowerCase().endsWith(".zip")) {
      ...
      System.out.println("it is NOT a ZIP file");
      mainFolder = (new java.io.File(Variables.CACHE_PATH + zipFile)).getAbsolutePath();
    }
    return mainFolder;
  }
  ...
  private boolean removeFromDownloadedList(String urlToRemoveFromDownloadedList) {
    System.out.println(
        "fn name: private CacheManager.removeFromDownloadedList(); params: urlToRemoveFromDownloadedList '"
            + urlToRemoveFromDownloadedList + "'");
  ...
  private boolean updateCache() {
    ...
    if (list_recentDownloads.size() > Variables.DISK_CACHE_NUMBER_LIMIT
